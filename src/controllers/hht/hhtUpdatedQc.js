...
export const updateMovementType = async (req, res) => {
  ...
  console.log(
    chalk.yellow(`📦 Processing ${palletBarcodeArray.length} pallet(s)`)
  );

  if (palletBarcodeArray.length !== qtyArray.length) {
    console.log(
      chalk.red.bold('❌ ERROR: PalletBarcodes and Qtys length mismatch')
    );
    console.log(chalk.red(`   - Pallets count: ${palletBarcodeArray.length}`));
    console.log(chalk.red(`   - Quantities count: ${qtyArray.length}`));
    return res
      .status(400)
      .json({ error: 'PalletBarcodes and Qtys length mismatch' });
  }
  ...
  try {
    ...
    if (Remark === 'Unrestricted') {
      ...
      if (!grResult.success) {
        ...
        console.log(
          chalk.green(
            `   Material Document: ${grResult.materialDocument || 'N/A'}`
          )
        );
        console.log(
          chalk.green(`   Processed Count: ${grResult.processedCount || 0}`)
        );
      }
      ...
      console.log(
        chalk.cyan(
          '🔍 Analyzing pallet details and determining transfer requirements...'
        )
      );
      ...
      for (let i = 0; i < palletBarcodeArray.length; i++) {
        ...
        console.log(
          chalk.gray(`      - Material: ${palletDetails.material || 'N/A'}`)
        );
        console.log(
          chalk.gray(`      - Order: ${palletDetails.orderNumber || 'N/A'}`)
        );
        console.log(
          chalk.gray(`      - Batch: ${palletDetails.batch || 'N/A'}`)
        );
        console.log(
          chalk.gray(
            `      - Storage Location: ${palletDetails.storageLocation || 'N/A'}`
          )
        );
        console.log(
          chalk.gray(`      - QC Status: ${palletDetails.qcStatus || 'N/A'}`)
        );
        ...
        const paddedOrderNumber = orderNumber
          ? orderNumber.padStart(12, '0')
          : '';
        ...
        if (
          ...
          console.log(
            chalk.yellow(
              `   ⚡ Two-step transfer required (5190 → Unrestricted → 5110)`
            )
          );
          needsTransfer.push({
            index: i,
          ...
          console.log(
            chalk.magenta(
              `      Step 1: Adding 321 movement (5190 unrestricted)`
            )
          );
          ...
          console.log(
            chalk.magenta(`      Step 2: Adding 311 movement (5190 → 5110)`)
          );
          move5110Items.push({
            MATERIAL: paddedMaterial,
            PLANT: PRODUCTION_PLANT,
          ...
          console.log(
            chalk.green(`   ✅ Standard transfer (direct to 5110 unrestricted)`)
          );
          goodsmvtItems.push({
            MATERIAL: paddedMaterial,
            PLANT: PRODUCTION_PLANT,
      ...
      console.log(
        chalk.gray(`   - Two-step transfers needed: ${needsTransfer.length}`)
      );
      console.log(
        chalk.gray(`   - Transfer to 5190 items: ${transfer5190Items.length}`)
      );
      console.log(
        chalk.gray(`   - Move to 5110 items: ${move5110Items.length}`)
      );
      console.log(
        chalk.gray(`   - Direct unrestricted items: ${goodsmvtItems.length}`)
      );
      ...
      if (transfer5190Items.length > 0) {
        ...
        console.log(
          chalk.gray(`📤 Sending ${transfer5190Items.length} items to SAP...`)
        );
        ...
        if (!firstTxSuccess) {
          console.log(
            chalk.red.bold('❌ Step 1 Failed: Transfer to 5190 Unrestricted')
          );
          console.log(
            chalk.red(`   Error: ${sapResponse1.Message || 'Unknown error'}`)
          );
          sapSuccess = false;
          sapMessage +=
            sapResponse1.Message ||
            'Failed to transfer to unrestricted in 5190';

          // Log errors for first step (321 movement)
          console.log(
            chalk.yellow(
              '📝 Logging errors for failed first transaction (321)...'
            )
          );
          ...
          console.log(
            chalk.yellow(
              '📝 Logging errors for second transaction (311) due to first transaction failure...'
            )
          );
          for (const item of needsTransfer) {
            await logSAPQCError(
          ...
          console.log(
            chalk.green('✅ Step 1 Successful: Transfer to 5190 Unrestricted')
          );
          console.log(
            chalk.green(
              `   Material Document: ${sapResponse1.materialDocument || 'N/A'}`
            )
          );
          ...
          console.log(
            chalk.gray(`📤 Sending ${move5110Items.length} items to SAP...`)
          );
          ...
          if (!secondTxSuccess) {
            console.log(
              chalk.red.bold('❌ Step 2 Failed: Move from 5190 to 5110')
            );
            console.log(
              chalk.red(`   Error: ${sapResponse2.Message || 'Unknown error'}`)
            );
            sapSuccess = false;
            sapMessage +=
            ...
            console.log(
              chalk.yellow(
                '📝 Logging errors for failed second transaction (311)...'
              )
            );
            for (const item of needsTransfer) {
              await logSAPQCError(
            ...
            console.log(
              chalk.green('✅ Step 2 Successful: Move from 5190 to 5110')
            );
            console.log(
              chalk.green(
                `   Material Document: ${sapResponse2.materialDocument || 'N/A'}`
              )
            );
            console.log(
              chalk.green(
                `   Document Year: ${sapResponse2.documentYear || 'N/A'}`
              )
            );
      ...
      if (goodsmvtItems.length > 0) {
        ...
        console.log(
          chalk.gray(
            `📤 Sending ${goodsmvtItems.length} direct items to SAP...`
          )
        );
        ...
        if (sapResponse.Status === 'F') {
          console.log(
            chalk.red.bold('❌ Direct Processing Failed: QC Unrestricted')
          );
          console.log(
            chalk.red(`   Error: ${sapResponse.Message || 'Unknown error'}`)
          );
          sapSuccess = false;
          ...
          console.log(
            chalk.yellow('📝 Logging errors for failed direct transfers...')
          );
          for (let i = 0; i < palletBarcodeArray.length; i++) {
            ...
            const currentStorageLocation =
              palletDetails.storageLocation === '5190' ? '5190' : '5110';
          ...
          console.log(
            chalk.green('✅ Direct Processing Successful: QC Unrestricted')
          );
          console.log(
            chalk.green(
              `   Material Document: ${sapResponse.materialDocument || 'N/A'}`
            )
          );
          console.log(
            chalk.green(
              `   Document Year: ${sapResponse.documentYear || 'N/A'}`
            )
          );
      ...
      console.log(
        chalk.cyan(
          '🔍 Analyzing pallet details and determining blocking requirements...'
        )
      );
      ...
      for (let i = 0; i < palletBarcodeArray.length; i++) {
        ...
        if (
          ...
          console.log(
            chalk.yellow(
              `   ⚠️  Special Case: Pallet in 5190 Q stock - only needs movement 350`
            )
          );
          ...
          console.log(
            chalk.red(
              `   🚫 Standard Case: Adding blocked movement items (350 & 325)`
            )
          );
          qcStatusItems.push({
            MATERIAL: paddedMaterial,
      ...
      console.log(
        chalk.gray(
          `   - Simple block (5190 Q to S): ${simpleBlockItems.length}`
        )
      );
      console.log(
        chalk.gray(`   - Standard QC status items: ${qcStatusItems.length}`)
      );
      console.log(
        chalk.gray(`   - Standard transfer items: ${stockTransferItems.length}`)
      );
      ...
      if (qcStatusItems.length > 0) {
        ...
        if (!firstTxSuccess) {
          ...
          console.log(
            chalk.yellow(
              '📝 Logging errors for failed standard block step 1...'
            )
          );
          for (let i = 0; i < palletBarcodeArray.length; i++) {
        ...
        if (stockTransferItems.length > 0) {
          ...
          if (!secondTxSuccess) {
            ...
            console.log(
              chalk.yellow(
                '📝 Logging errors for failed standard block step 2...'
              )
            );
            for (let i = 0; i < palletBarcodeArray.length; i++) {
              // Skip items that were in the special case (5190 Q to S)
              ...
              const palletDetails = await getPalletSAPDetails(pallet);
              const materialCode =
                palletDetails.material || pallet.split('|')[0];
              const orderNumber = palletDetails.orderNumber || '';
              const batch = palletDetails.batch || '';
      ...
      if (!firstTxSuccess || !secondTxSuccess) {
        ...
        if (!firstTxSuccess && !secondTxSuccess) {
          ...
          sapMessage +=
            'First SAP transaction (344) failed: ' +
            sapResponse1.Message +
            '; ';
        } else {
          sapMessage +=
            'Second SAP transaction (325) failed: ' +
            sapResponse2.Message +
            '; ';
        }
        ...
        materialDocument = secondTxSuccess
          ...
          : firstTxSuccess
            ...
            : null;
        documentYear = secondTxSuccess
          ? sapResponse2.documentYear
          : firstTxSuccess
            ? sapResponse1.documentYear
            : null;
      } else {
        materialDocument = sapResponse2.materialDocument;
        documentYear = sapResponse2.documentYear;
      ...
      if (!firstTxSuccess || !secondTxSuccess) {
        ...
        if (!firstTxSuccess && !secondTxSuccess) {
          ...
          sapMessage +=
            'First SAP transaction (325) failed: ' +
            sapResponse1.Message +
            '; ';
        } else {
          sapMessage +=
            'Second SAP transaction (343) failed: ' +
            sapResponse2.Message +
            '; ';
        }
    ...
    if (sapSuccess) {
      ...
      console.log(
        chalk.green(`📄 Material Document: ${materialDocument || 'N/A'}`)
      );
...
async function sendToSAP(requestBody) {
  try {
    ...
    console.log(
      chalk.gray(
        `   URL: ${SAP_CONNECTOR_MIDDLEWARE_URL}/api/goods-movement/create`
      )
    );
    console.log(
      chalk.gray(`   GM_CODE: ${requestBody.GOODSMVT_CODE?.GM_CODE || 'N/A'}`)
    );
    console.log(
      chalk.gray(`   Items Count: ${requestBody.GOODSMVT_ITEM?.length || 0}`)
    );
    ...
    if (data.Return && data.Return.length > 0) {
      ...
      console.log(
        chalk.yellow(
          `⚠️  SAP Return Message: ${returnMessage.TYPE} - ${returnMessage.MESSAGE}`
        )
      );
    ...
    const materialDocument =
      data.MaterialDocument || data.GoodsMovementHeadRet?.MAT_DOC || null;
    console.log(
      chalk.green(
        `✅ SAP Success: Material Document ${materialDocument || 'N/A'}`
      )
    );
    return {
      Status: 'T',
      materialDocument: materialDocument,
      documentYear:
        data.MatDocumentYear || data.GoodsMovementHeadRet?.MATDOCUMENTYEAR,
    };
  } catch (error) {
...
export const getPendingSAPTransaction = async (req, res) => {
  ...
  try {
    ...
    console.log(
      chalk.green(
        `✅ Query successful - Found ${result.length} pending transactions`
      )
    );
    res.json(result);
  } catch (error) {
...
export const qcStatusChecking = async (req, res) => {
  ...
  try {
    try {
      ...
      return res
        .status(500)
        .json({ Status: 'F', Message: 'Internal server error' });
    }
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ Status: 'F', Message: 'Failed to check QC status' });
  }
};
...
async function getPalletSAPDetails(palletBarcode) {
  try {
    ...
    if (palletDetails && palletDetails.length > 0) {
      ...
      console.log(
        chalk.yellow(`⚠️  No pallet details found for: ${palletBarcode}`)
      );
      return {
        orderNumber: '',
        material: '',
    ...
    console.log(
      chalk.red.bold(`❌ Error retrieving pallet details for ${palletBarcode}:`)
    );
    console.log(chalk.red(`   Error: ${error.message}`));
    return {
      orderNumber: '',
...
async function processGoodReceipt(
  palletBarcodes,
  qtys,
  units,
  unitIsos,
  ...
  try {
    ...
    for (let i = 0; i < palletBarcodes.length; i++) {
      ...
      console.log(
        chalk.cyan(
          `🔍 Processing pallet ${i + 1}/${palletBarcodes.length}: ${palletBarcode}`
        )
      );
      ...
      if (palletBarcode.includes('#') && palletBarcode.includes(';')) {
        ...
        if (palletDetails && palletDetails.success) {
          ...
          console.log(
            chalk.red(`❌ Failed to get pallet details for: ${pallet}`)
          );
        }
      } else {
        console.log(
          chalk.red(`❌ Invalid pallet barcode format: ${palletBarcode}`)
        );
      }
    }
    ...
    console.log(
      chalk.blue(
        `📤 Sending ${goodsmvtItems.length} items to SAP for GR processing...`
      )
    );
    ...
    if (response.data.Return && response.data.Return.length > 0) {
      ...
      if (['E', 'I', 'A'].includes(returnMessage.TYPE)) {
        ...
        console.log(
          chalk.yellow(
            '📝 Updating database with empty material document due to SAP error...'
          )
        );
        for (const palletData of allPalletData) {
          try {
            await executeQuery(
              `EXEC [dbo].[HHT_FG_GR_BarcodeUpdate_New] @PalletBarcode, @MaterialDocument, @UserId`,
              [
                {
                  name: 'PalletBarcode',
                  type: sql.NVarChar(50),
            ...
            console.log(
              chalk.gray(`   ✅ Updated pallet: ${palletData.palletBarcode}`)
            );
          } catch (updateError) {
            console.error(
              chalk.red(
                `   ❌ Error updating pallet ${palletData.palletBarcode}:`,
                updateError
              )
            );
          }
    ...
    if (!materialDocument) {
      ...
      console.log(
        chalk.yellow(
          '📝 Updating database with empty material document due to no material document...'
        )
      );
      for (const palletData of allPalletData) {
        try {
    ...
    console.log(
      chalk.green(`✅ SAP GR Success: Material Document ${materialDocument}`)
    );

    // Update pallet records in database with successful material document
    console.log(
      chalk.blue('💾 Updating database with successful material document...')
    );
    for (const palletData of allPalletData) {
      try {
        await executeQuery(
    ...
    if (allPalletData && allPalletData.length > 0) {
      console.log(
        chalk.yellow(
          '📝 Updating database with empty material document due to network error...'
        )
      );
      for (const palletData of allPalletData) {
        try {
      ...
      console.log(
        chalk.yellow(
          '⚠️  No pallet data available to update in database due to early error'
        )
      );
    }
    ...
    if (goodsmvtItems && goodsmvtItems.length > 0) {
      console.log(
        chalk.yellow('📝 Logging network errors for constructed SAP items...')
      );
      for (const item of goodsmvtItems) {
        await logSAPQCError(
          item.ITEM_TEXT,
      ...
      console.log(
        chalk.yellow('📝 Logging network errors for processed pallets...')
      );
      for (const palletData of allPalletData) {
        // Determine storage location based on batch (same logic as GR)
        const storageLocation =
          palletData.batch && palletData.batch.includes('RS') ? '5190' : '5110';
      ...
      console.log(
        chalk.yellow('📝 Logging network errors for input pallets...')
      );
      for (let i = 0; i < palletBarcodes.length; i++) {
        const palletBarcode = palletBarcodes[i];
        if (palletBarcode.includes('#') && palletBarcode.includes(';')) {
