...
export const editHHTRegisterStatus = async (req, res) => {
  ...
  try {
    const result = await executeQuery(
      'EXEC [dbo].[hht_register_edit_status] @device_sn, @register_by, @mobile_no',
      [
...
export const getHHTRegisterSpecific = async (req, res) => {
  ...
  try {
    const result = await executeQuery(
      'EXEC [dbo].[hht_register_select_specific] @name, @mobile_no, @device_status',
      [
...
export const getAllRegisterHHTDevice = async (req, res) => {
  try {
    ...
    const result = await executeQuery(
      'EXEC [dbo].[sp_get_all_register_hht_device]'
    );
...
export const addHHTRegisterRequest = async (req, res) => {
  ...
  try {
    const result = await executeQuery(
      'EXEC [dbo].[hht_register_insert] @device_sn, @mobile_no, @request_by',
      [
        { name: 'device_sn', type: sql.NVarChar(50), value: device_sn },
        { name: 'mobile_no', type: sql.NVarChar(50), value: mobile_no },
...
export const updateAndroidAccess = async (req, res) => {
  ...
  try {
    const result = await executeQuery(
      'EXEC [dbo].[sp_android_access_update] @user_id, @user_name, @approved_by',
      [
...
export const getPendingApprovals = async (req, res) => {
  try {
    const result = await executeQuery(
      'EXEC [dbo].[sp_android_access_approval_pending]',
      []
    );
    res.json(result);
  } catch (error) {
