...
router.get(
  '/loc-wise-item-qty',
  auth,
  Location2DController.getLocationWiseItemQty
);
...
router.get(
  '/material-in-stock',
  auth,
  MaterialInStockController.getMaterialInStock
);

// monthly box and pallet count
router.get(
  '/monthly-wise-print-count',
  auth,
  TotalPrintMonthWiseController.getBoxAndPalletCountByMonth
);
router.post('/details', DashBoardController.dashboardDetails);
router.get(
  '/put-pick-last-6-month',
  TotalPrintMonthWiseController.pickVsPutLastSixMonth
);
router.get(
  '/put-pick-last-7-day',
  TotalPrintMonthWiseController.Sp_PutVsPick_Last7Days
);
